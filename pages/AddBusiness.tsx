...
const AddBusiness: React.FC<AddBusinessProps> = ({ user, login }) => {
  ...
  const addJob = () => {
    ...
    setJobs([...jobs, newJob]);
    setCurrentJob({ title: '', description: '', type: JobType.FULL_TIME, salary: '' });
    setAppValue('');
                ...
                <div className="space-y-4">
                   {jobs.length > 0 ? (
                      <div className="grid grid-cols-1 gap-4">
                         {jobs.map((job, idx) => (
                           <div key={idx} className="bg-white border-2 border-stone-100 p-6 rounded-[2rem] flex items-center justify-between group hover:border-sky-200 transition-all">
                              <div className="flex items-center space-x-6">
                                 <div className="bg-sky-50 p-4 rounded-2xl text-sky-900">
