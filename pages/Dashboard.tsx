...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const [jobs, setJobs] = useState([
    { id: 'j1', title: 'Surf Instructor', type: JobType.PART_TIME, salary: '$25/hr', apps: 14, posted: '2 days ago' },
    { id: 'j2', title: 'Retail Associate', type: JobType.FULL_TIME, salary: '$18-20/hr', apps: 8, posted: '1 week ago' }
  ]);
  ...
  const stats = [
    ...
    { label: 'Profile Views', value: '3,452', growth: '+8.2%', icon: Users, color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'Link Clicks', value: '892', growth: '+24.1%', icon: MousePointer2, color: 'text-orange-600', bg: 'bg-orange-50' },
    { label: 'Conversion Rate', value: '4.2%', growth: '+18.5%', icon: Zap, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  ];
  ...
  return (
    <div className="min-h-screen bg-stone-50 pt-28 pb-20 px-4">
      ...
      {/* Edit Profile Modal (Manual Editing Suite) */}
      {isEditProfileOpen && (
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-8 md:p-12 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Website URL</label>
                  <input 
                    type="url" 
                    className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-6 py-4 outline-none font-bold text-sky-950"
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  <Save className="h-5 w-5 mr-2" /> Save Manual Changes
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Other Modals (Deals, Events, Jobs) */}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[3.5rem] shadow-xl border border-stone-100 relative overflow-hidden">
          ...
          <div className="flex gap-3 relative z-10">
            <button 
              ...
              className="p-4 bg-stone-100 text-sky-950 rounded-2xl hover:bg-stone-200 transition-all shadow-sm border border-stone-200 flex items-center"
              title="Manual Edit Profile"
            >
              <Edit3 className="h-5 w-5 mr-2" /> <span className="text-xs font-black uppercase tracking-widest hidden sm:inline">Manual Edit</span>
            </button>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 bg-white rounded-[3.5rem] shadow-xl border border-stone-100 p-10 space-y-10">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-2xl font-black text-sky-950 tracking-tight">Performance Trends</h2>
                      <p className="text-slate-400 text-sm font-medium">Daily profile impressions over the last 14 days</p>
                    </div>
                    <div className="flex items-center space-x-4">
                       <div className="flex items-center space-x-2 text-xs font-bold text-slate-500">
                          <div className="w-3 h-3 bg-sky-900 rounded-full"></div>
                          <span>Direct</span>
                       </div>
                       <div className="flex items-center space-x-2 text-xs font-bold text-slate-500">
                  ...
                  <div className="h-64 flex items-end justify-between px-4 pb-12 border-b border-stone-50 gap-2">
                    {[45, 67, 43, 89, 55, 92, 120, 110, 85, 95, 140, 115, 130, 155].map((h, i) => (
                      <div key={i} className="flex-grow flex flex-col justify-end space-y-1 group relative">
                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-sky-950 text-white px-2 py-1 rounded text-[10px] font-black opacity-0 group-hover:opacity-100 transition-opacity">
                          {Math.round(h * 1.5)}
                        </div>
                  ...
                  <div className="grid grid-cols-3 gap-6 pt-4 text-center">
                    ...
                    <div className="p-6 bg-stone-50 rounded-3xl">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Peak Day</div>
                      <div className="text-3xl font-black text-sky-950">Saturday</div>
                    </div>
                    <div className="p-6 bg-stone-50 rounded-3xl">
          ...
          {activeView === 'deals' && (
            <div className="bg-white rounded-[3.5rem] shadow-xl border border-stone-100 p-10 md:p-14 space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h2 className="text-3xl font-black text-sky-950 tracking-tight">Promotional Deals</h2>
                  <p className="text-slate-400 font-medium">Manage active coupons and community discounts.</p>
                </div>
                <button onClick={() => setIsDealModalOpen(true)} className="px-10 py-5 bg-sky-900 text-white font-black rounded-2xl hover:bg-sky-800 transition-all shadow-xl shadow-sky-900/20 flex items-center active:scale-95">
                  <PlusCircle className="mr-3 h-5 w-5" /> Create New Deal
                </button>
              </div>
          ...
          {activeView === 'events' && (
            <div className="bg-white rounded-[3.5rem] shadow-xl border border-stone-100 p-10 md:p-14 space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h2 className="text-3xl font-black text-sky-950 tracking-tight">Community Events</h2>
                  <p className="text-slate-400 font-medium">Coordinate workshops, launches, and meetups.</p>
                </div>
                <button onClick={() => setIsEventModalOpen(true)} className="px-10 py-5 bg-sky-900 text-white font-black rounded-2xl hover:bg-sky-800 transition-all shadow-xl shadow-sky-900/20 flex items-center active:scale-95">
                  <PlusCircle className="mr-3 h-5 w-5" /> Host Event
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {events.map(event => (
                  <div key={event.id} className="p-10 bg-stone-50 rounded-[3rem] border border-stone-100 flex flex-col justify-between group hover:border-sky-200 transition-all">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center space-x-3">
                           ...
                           <div>
                             <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Next Occurence</div>
                             <div className="text-sm font-black text-sky-950">{new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} @ {event.time}</div>
                           </div>
                         </div>
                      ...
                      <div className="flex items-center space-x-6">
                         <div className="flex items-center text-xs font-bold text-slate-500">
                            <Users className="h-4 w-4 mr-1.5 text-sky-700" /> {event.attendees} Confirmed RSVPs
                         </div>
                         <div className="flex items-center text-xs font-bold text-slate-500">
                    ...
                    <div className="flex items-center space-x-3 mt-8 pt-8 border-t border-stone-200">
                      <button className="flex-grow py-4 bg-white border border-stone-200 text-sky-900 font-bold rounded-xl hover:bg-stone-100 transition-all flex items-center justify-center"><Edit3 className="h-4 w-4 mr-2" /> Details</button>
          ...
          {activeView === 'jobs' && (
            <div className="bg-white rounded-[3.5rem] shadow-xl border border-stone-100 p-10 md:p-14 space-y-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h2 className="text-3xl font-black text-sky-950 tracking-tight">Talent Acquisition</h2>
                  <p className="text-slate-400 font-medium">Connect with local professionals and job seekers.</p>
                </div>
                <button onClick={() => setIsJobModalOpen(true)} className="px-10 py-5 bg-sky-900 text-white font-black rounded-2xl hover:bg-sky-800 transition-all shadow-xl shadow-sky-900/20 flex items-center active:scale-95">
                  <PlusCircle className="mr-3 h-5 w-5" /> Post Opportunity
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {jobs.map(job => (
                  <div key={job.id} className="p-10 bg-stone-50 rounded-[3rem] border border-stone-100 flex flex-col justify-between group hover:border-sky-200 transition-all">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center space-x-3">
                           <div className="bg-sky-50 p-3 rounded-2xl text-sky-900 border border-sky-100"><Briefcase className="h-5 w-5" /></div>
                           <div>
                             <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Job Category</div>
                             <div className="text-sm font-black text-sky-950">{job.type}</div>
                           </div>
                         </div>
                         <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Posted {job.posted}</div>
                      </div>
                      <div>
                      ...
                      <div className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 flex items-center justify-between">
                         <div className="flex items-center">
                            ...
                            <span className="text-sm font-black text-sky-950">Active Applications</span>
                         </div>
                         <div className="text-2xl font-black text-sky-900">{job.apps}</div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3 mt-8 pt-8 border-t border-stone-200">
                      <button className="flex-grow py-4 bg-sky-900 text-white font-black rounded-xl hover:bg-sky-800 transition-all flex items-center justify-center shadow-lg active:scale-95">Review Applicants</button>
                      <button className="p-4 bg-white border border-stone-200 text-slate-400 rounded-xl hover:bg-stone-50 hover:text-sky-900 transition-all"><Edit3 className="h-5 w-5" /></button>
                    </div>
                  </div>
                ))}
              </div>
